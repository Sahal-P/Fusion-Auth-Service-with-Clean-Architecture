...
class UserViewSet(ViewSet):
    ...
    def login(self, request: Request, *args, **kwargs) -> Response:
        ...
        response.set_cookie(
            key="Access_token", value=payload["token"], secure=True, httponly=True
        )
        response.set_cookie(
            key="Refresh_token", value=refresh["token"], secure=True, httponly=True
        )
    ...
    def refresh_token(self, request: Request, *args, **kwargs) -> Response:
        ...
        if status == 201:
            response.set_cookie(
                key="Access_token", value=payload["token"], httponly=True
            )
        return response
