    ...
    token = jwt.encode(
        {
            ...
            + datetime.timedelta(seconds=JWT_ACCESS_EXP_DELTA_SECONDS),
    ...
    refresh = jwt.encode(
        {
            "user_id": id,
            "exp": datetime.datetime.utcnow()
            + datetime.timedelta(days=JWT_REFRESH_DAYS),
            "iat": datetime.datetime.utcnow(),
        },
        secret,
        algorithm=JWT_ALGORITHM,
...
def status_code_handler(exc, context):
    ...
    if isinstance(exc, Http404):
        return Response(
            {"detail": "mmmPage not found."}, status=status.HTTP_404_NOT_FOUND
        )
