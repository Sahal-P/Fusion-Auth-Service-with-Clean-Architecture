...
class UserController:
    ...
    def login(self, params: dict) -> Tuple[dict, int]:
        ...
        if "errors" in data:
            logger.error(
                f"{Fore.RED}Error Deserializing params: %s", str(data["errors"])
            )
    ...
    def register(self, params: dict) -> Tuple[dict, int]:
        ...
        try:
            ...
            # EntityDuplicate
            logger.error(
                "Error creating duplicate user with params %s: %s",
                str(params),
                err.message,
            )
    ...
    def refresh_token(self, params) -> Tuple[dict, int]:
        ...
        try:
            ...
            logger.error(
                "Error verifying refresh with params %s: %s", str(params), err.message
            )
            return {"error": err.message}, HTTPStatus.BAD_REQUEST.value
